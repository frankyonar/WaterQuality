<?php
require 'config.php';
require 'mailsender.php'; // Includi il file mailsender.php per avere accesso alla funzione send_mail
session_start();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($user) {
        ...
        $new_password = bin2hex(random_bytes(4)); // Password temporanea di 8 caratteri
        $hashed_password = password_hash($new_password, PASSWORD_BCRYPT);

        // Aggiorna la password nel database
        $stmt = $conn->prepare("UPDATE utente SET Password = ? WHERE ID_Utente = ?");
        $stmt->execute([$hashed_password, $user['ID_Utente']]);

        // Prepara i dettagli dell'email
        $oggetto = "Recupero Password";
        $messaggio = "La tua nuova password temporanea è: " . $new_password;

        // Invia l'email utilizzando la funzione send_mail direttamente
        $send_mail_result = send_mail($email, $oggetto, $messaggio);

        if ($send_mail_result) {
            $success_message = "Email con la tua nuova password è stata inviata.";
        } else {
...
<head>
    ...
    <title>Forgotten Password - Water quality</title>
    ...
    <div class="container"><img width="197" height="111" src="assets/img/brands/Water%20quality.svg"><button data-bs-toggle="collapse" class="navbar-toggler" data-bs-target="#navcol-1"><span class="visually-hidden">Toggle navigation</span><span class="navbar-toggler-icon"></span></button>
        <div class="collapse navbar-collapse" id="navcol-1">
            <ul class="navbar-nav mx-auto">
                ...
                <li class="nav-item"><a class="nav-link" href="registrati.php"></a></li>
    ...
    <div class="container py-md-5">
        <div class="row d-flex align-items-center">
            ...
            <div class="col-md-5 col-xl-4 text-center text-md-start">
                <h2 class="display-6 fw-bold mb-4">Hai dimenticato la&nbsp;<span class="underline">password</span>?</h2>
                <p class="text-muted">Non preoccuparti, ora la recuperiamo.</p>
